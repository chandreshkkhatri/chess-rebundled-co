...
const LOCAL_PARSE_CONFIDENCE_THRESHOLD = 0.7;
...
export interface AIParseResult {
  ...
  transcript: string;
  confidence: number;
  alternatives: Array<{ text: string; confidence: number }> | string[];
  reasoning?: string;
}

export type VoiceInputMode = "text-only" | "tap-only" | "voice-tap";
export type VoiceParsingMode = "webspeech-haiku" | "gemini-audio";

export interface UniversalInputPanelProps {
  // Game State
  fen: string;
  isActive: boolean;
  ...
  // Custom feedback/status
  pendingOpponentMove?: { san: string } | null;
  lastMoveResult?: { isCorrect: boolean; expectedMove: string } | null;

  // AI Parsing State from Parent
  ...
  onParseMoveWithText: (text: string, rawTranscript?: string) => void;
...
export function UniversalInputPanel({
  fen,
  isActive,
  isSubmitting,
  inputMode,
  voiceParsingMode,
  autoSubmitEnabled,
  autoSubmitConfidenceThreshold = AUTO_SUBMIT_CONFIDENCE_THRESHOLD,
  pendingOpponentMove,
  lastMoveResult,
  aiParseResult,
  aiParseError,
  ...
  // Track raw transcript from Web Speech for display
  const [rawTranscript, setRawTranscript] = useState<string | null>(null);

  const autoListenTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const autoSubmitTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const lastProcessedRef = useRef<{ move: string; timestamp: number } | null>(
    null,
  );
  ...
  const stageOptions = useMemo(() => {
    ...
    switch (stageState.stage) {
      ...
      case "disambiguation":
        ...
        if (!stageState.selectedPiece || !stageState.selectedDestination)
          return [];
        return (
          getDisambiguationOptions(
            legalMoves,
      ...
      case "destination": {
        ...
        const pieceOrFile =
          stageState.selectedPawnFile || stageState.selectedPiece;
    ...
    (piece: string) => {
      ...
      if (piece === "P") {
        ...
        if (pawnMoves.length === 1) {
          ...
          const fullMove =
            legalMoves.find((m) => m.replace(/[+#]/g, "") === pawnMoves[0]) ||
            pawnMoves[0];
          clearLastMoveResult();
  ...
  const handleDestinationSelect = useCallback(
    (destination: string) => {
      ...
      if (stageState.selectedPiece === "P") {
        ...
        // Find the actual legal move (may have +/# suffix)
        const fullMove =
          legalMoves.find((m) => m.replace(/[+#]/g, "") === destination) ||
          destination;
        clearLastMoveResult();
        setSelectedMove(fullMove);
        onMoveSubmit(fullMove, 1.0);
  ...
  const handleStageOptionSelect = useCallback(
    (option: string) => {
      switch (stageState.stage) {
        case "piece":
  ...
  const handleVoiceResult = useCallback(
    (parsedMove: string, confidence: number, rawTranscriptText?: string) => {
      if (
        !isSubmitting &&
        !isAIParsing &&
        isActive &&
        ...
        if (
          selectedMove &&
          isLegalMove(selectedMove) &&
          !isLegalMove(parsedMove)
        ) {
          return;
        }
        ...
        // Store raw transcript for display
        ...
        clearAIParseState();
        if (clearLastMoveResult) clearLastMoveResult();
        setSelectedMove(null);

        // Always surface a local parse immediately so the UI feels responsive.
        // Send both parsed move and raw transcript so backend has full context.
        onParseMoveWithText(parsedMove, rawTranscriptText);

        // Auto-submit for high confidence legal moves (if enabled)
        if (
          autoSubmitEnabled &&
          confidence >= autoSubmitConfidenceThreshold &&
          isLegalMove(parsedMove)
        ) {
  ...
  useEffect(() => {
    if (aiParseResult?.parsedMove && !isSubmitting && isActive) {
      ...
      if (
        autoSubmitEnabled &&
        aiParseResult.confidence >= autoSubmitConfidenceThreshold &&
  ...
  const isSupported =
    voiceParsingMode === "webspeech-haiku"
      ...
      : isGeminiSupported;
  const error =
    voiceParsingMode === "webspeech-haiku" ? webSpeechError : geminiError;
    ...
    return (
      <div className="bg-yellow-900/50 border border-yellow-700 rounded-lg p-2">
        <div className="text-yellow-200 text-xs">
          Voice recognition not supported in this browser. Use Chrome for voice
          input, or change to &quot;Tap Only&quot; mode in Settings.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg p-1.5 h-full flex flex-row gap-2 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
        {/* ZONE 1: Status Bar - Heard/Interpreted/Alternatives or Processing/Error */}
        <div className="min-h-[20px] flex-shrink-0 flex items-center justify-center">
          {!isActive && pendingOpponentMove ? (
            <div className="text-amber-200 text-xs font-medium">
              Opponent played:{" "}
              <span className="font-mono">{pendingOpponentMove.san}</span>
            </div>
          ) : isAIParsing ? (
              ...
              {(rawTranscript || aiParseResult.transcript) && (
                <div className="text-slate-500 truncate">
                  ...
                  <span className="italic">
                    &ldquo;{rawTranscript || aiParseResult.transcript}&rdquo;
                  </span>
                </div>
              )}
              <div className="text-slate-400">
                <span className="text-slate-500">Interpreted:</span>{" "}
        ...
        <div
          className={`flex-1 min-h-0 ${tapGridEnabled && isActive ? "" : "invisible"}`}
        >
              ...
              {(stageOptions.length > 0
                ...
                return (
                  <button
                    key={
                      stageOptions.length > 0
                        ? `${stageState.stage}-${option}`
                        : idx
                    }
                    onClick={() =>
                      option !== "—" && handleStageOptionSelect(option)
                    }
                    disabled={isSubmitting || option === "—"}
                    aria-label={
                      stageState.stage === "piece"
                        ? getPieceAriaLabel(option)
                        : option
                    }
        ...
        <div className="h-8 flex-shrink-0 flex items-center justify-center">
          {lastMoveResult ? (
            <div
              className={`w-full text-center py-1 px-2 rounded text-xs font-bold uppercase tracking-wide ${
      ...
      <div className="flex flex-col gap-2 flex-shrink-0 w-20">
        <button
          ...
          className={`h-10 rounded-lg font-bold text-xs transition-colors uppercase tracking-wide ${
        ...
        <button
          ...
          className={`relative flex-1 rounded-lg font-bold transition-all shadow-lg flex flex-col items-center justify-center ${
            isSubmitting
