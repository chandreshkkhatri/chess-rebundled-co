...
export interface UniversalInputPanelProps {
  ...
  onParseMoveWithText: (text: string) => void;
...
export function UniversalInputPanel({
  ...
  const handleVoiceResult = useCallback((parsedMove: string, confidence: number) => {
    if (!isSubmitting && !isAIParsing && isActive && voiceParsingMode === 'webspeech-haiku') {
      const now = Date.now();
      ...
      // If confidence is low, we can still refine with backend API in the background.
      onParseMoveWithText(parsedMove); 
  ...
  }, [onParseMoveWithText, isSubmitting, isAIParsing, isActive, clearAIParseState, clearLastMoveResult, voiceParsingMode, selectedMove, isLegalMove, onMoveSubmit, autoSubmitEnabled]);

  // Handle audio ready from Gemini voice (Gemini mode)
  // Block new requests while already parsing to prevent race conditions
  const handleAudioReady = useCallback((audioBase64: string, mimeType: string) => {
    if (!isSubmitting && !isAIParsing && isActive && voiceParsingMode === 'gemini-audio') {
      clearAIParseState();
      clearLastMoveResult(); // Clear stale feedback from previous move
      setSelectedMove(null);
      onParseMoveWithAudio(audioBase64, mimeType);
    }
  }, [onParseMoveWithAudio, isSubmitting, isAIParsing, isActive, clearAIParseState, clearLastMoveResult, voiceParsingMode]);
