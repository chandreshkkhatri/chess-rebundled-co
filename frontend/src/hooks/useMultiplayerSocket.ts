...
export function useMultiplayerSocket() {
  ...
  useEffect(() => {
    ...
    if (!mpListenersAttached) {
      ...
      socket.on("mp-search-cancelled", () => {
        ...
        if (mpSearchTimeoutId) {
          clearTimeout(mpSearchTimeoutId);
          mpSearchTimeoutId = null;
        }
      });
      ...
      socket.on(
        "mp-invite-created",
        (data: { inviteCode: string; gameId: string }) => {
          const store = useMultiplayerStore.getState();
  ...
  const parseMoveWithAI = useCallback(
    (gameId: string, transcript: string, rawTranscript?: string) => {
      const socket = getSocket();
      useMultiplayerStore.getState().setAIParsing(true);
      socket.emit("mp-parse-move-with-ai", {
        gameId,
        transcript,
        rawTranscript,
      });
  ...
  const parseAudioMoveWithGemini = useCallback(
    (gameId: string, audioBase64: string, mimeType: string) => {
      const socket = getSocket();
      useMultiplayerStore.getState().setAIParsing(true);
      socket.emit("mp-parse-audio-move-with-gemini", {
        gameId,
        audioBase64,
        mimeType,
      });
